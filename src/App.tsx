...
const projects: Project[] = [
  {
    id: 'bingo',
    index: '01',
    title: 'Concurrent Bingo Server',
    category: 'NETWORKING / CONCURRENCY',
    summary:
      'Event-driven multiplayer server designed around bounded work queues, deterministic session state, and low-contention message dispatch.',
    metrics: [['500+', 'clients'], ['18 ms', 'p99 latency'], ['8', 'workers']],
    stack: ['C++20', 'TCP', 'Thread Pool', 'SQLite'],
    challenge: 'Maintain consistent game state while hundreds of clients produce bursty traffic.',
    solution:
      'Separated I/O from game-state mutation, assigned sessions to stable workers, and used bounded queues to apply backpressure before overload.',
    architecture: ['TCP Clients', 'I/O Reactor', 'Task Queue', 'Session Workers', 'State Store'],
  },
  {
    id: 'cuda',
    index: '02',
    title: 'CUDA Video Pipeline',
    category: 'GPU COMPUTE / VIDEO',
    summary:
      'A grayscale video processor that overlaps host-to-device transfer, kernel execution, and device-to-host transfer across CUDA streams.',
    metrics: [['4.8×', 'throughput'], ['240', 'FPS'], ['2.1 ms', 'kernel']],
    stack: ['CUDA', 'C++', 'OpenCV', 'Nsight'],
    challenge: 'The first implementation was transfer-bound and left the GPU idle between frames.',
    solution:
      'Pinned host memory, tiled kernels for coalesced access, and a three-stage stream pipeline keep compute and transfers overlapped.',
    architecture: ['Decode', 'Pinned Buffer', 'H→D Stream', 'CUDA Kernel', 'D→H Stream'],
  },
  {
    id: 'vfs',
    index: '03',
    ...
    summary:
      'An inode-based file system emulator with hierarchical directories, block allocation, permissions, and a diagnostic shell.',
    metrics: [['4 KiB', 'blocks'], ['O(1)', 'block lookup'], ['64 MiB', 'volume']],
    stack: ['C++', 'Inodes', 'Bitmaps', 'RAII'],
    challenge: 'Make allocation failure safe without corrupting metadata or leaking partially claimed blocks.',
    solution:
      'Transactional allocation stages mutations and commits only after every required block and inode has been validated.',
    architecture: ['CLI', 'VFS API', 'Inode Table', 'Block Bitmap', 'Virtual Disk'],
  },
  {
    id: 'crypto',
    index: '04',
    title: 'Streaming File Encryption',
    category: 'SECURITY / I/O',
    summary:
      'Authenticated, chunked file encryption designed to process inputs larger than available memory while preserving failure safety.',
    metrics: [['256-bit', 'key'], ['64 KiB', 'chunks'], ['0', 'plaintext copies']],
    stack: ['C++', 'AES-GCM', 'Streaming I/O', 'CMake'],
    challenge: 'Encrypt arbitrarily large files without whole-file buffering or partial output on authentication failure.',
    solution:
      'A bounded streaming pipeline writes to a temporary target and atomically promotes it only after tag verification succeeds.',
    architecture: ['Input Stream', 'Chunk Buffer', 'AES-GCM', 'Auth Tag', 'Atomic Output'],
  },
]

const skills = [
  ['LANGUAGES', 'C++20', 'C', 'CUDA', 'Python', 'C#'],
  ['SYSTEMS', 'Memory Management', 'Cache Locality', 'Threads', 'Synchronization', 'Processes'],
  ['GRAPHICS', 'OpenGL', 'GLSL', 'Render Pipelines', 'Framebuffers', 'Shader Design'],
  ['NETWORKING', 'TCP / UDP', 'Sockets', 'WebSockets', 'Protocols', 'Backpressure'],
  ['PERFORMANCE', 'Profiling', 'SIMD', 'Data Layout', 'Lock Contention', 'GPU Analysis'],
  ['ARCHITECTURE', 'Design Patterns', 'RAII', 'Testability', 'CMake', 'Clean APIs'],
]
...
function Icon({ name }: { name: 'arrow' | 'github' | 'linkedin' | 'mail' | 'download' }) {
  const paths = {
...
function Profiler() {
  ...
  return (
    <div className="profiler">
      <div className="profiler-head"><div><i /> LIVE PROFILER</div><span>FRAME #18,492</span></div>
      <div className="profiler-grid">
        <div className="gauge"><span>FPS</span><strong>238</strong><small>4.20 ms</small></div>
        <div className="gauge"><span>CPU</span><strong>37<em>%</em></strong><small>8 active threads</small></div>
        <div className="gauge"><span>GPU</span><strong>64<em>%</em></strong><small>1.8 GB VRAM</small></div>
        <div className="gauge"><span>MEM</span><strong>842<em>MB</em></strong><small>0 alloc spikes</small></div>
      </div>
      <div className="frame-chart">
        ...
        <div className="budget-line"><span>4.16 ms budget</span></div>
      </div>
      <div className="profile-rows">
        {[['RenderScene()', '1.42 ms', 72], ['UpdatePhysics()', '0.81 ms', 44], ['NetworkTick()', '0.24 ms', 18]].map(row =>
    ...
    <>
      ...
      <main>
        <section id="home" className="hero-section">
          ...
          <div className="hero-grid">
            <div className="hero-copy">
              ...
              <p className="lede">I build performance-critical C++ systems, real-time graphics, and concurrent networked software—with a focus on predictable behavior and clean architecture.</p>
              <div className="hero-actions">
                <button className="primary" onClick={() => go('#projects')}>EXPLORE SYSTEMS <Icon name="arrow" /></button>
                <a href="#contact">VIEW RÉSUMÉ <Icon name="download" /></a>
              </div>
            </div>
            <CpuVisual />
          </div>
          <div className="runtime-strip">
            ...
            <span>PRIMARY <b>C++20</b></span>
            <span>FOCUS <b>SYSTEMS + GRAPHICS</b></span>
        ...
        <section id="about" className="section about">
          ...
          <div className="terminal">
            ...
            <pre><b>$</b> whoami{'\n'}<span>Systems programmer focused on C++, graphics, networking, and performance.</span>{'\n\n'}<b>$</b> cat current_interests.txt{'\n'}<span>GPU architecture · lock-free data structures · real-time rendering · distributed systems</span>{'\n\n'}<b>$</b> echo $ENGINEERING_GOAL{'\n'}<em>"Build software whose performance and correctness are explainable."</em></pre>
          </div>
        </section>
            ...
            {projects.map(project => (
              <article key={project.id} onClick={() => setSelected(project)}>
                ...
                <div className={`project-diagram ${project.id}`}>
                  ...
                  {project.id === 'crypto' && <div className="crypto-flow"><b>PLAINTEXT</b><i>256</i><span>AES–GCM</span><i>TAG</i><b>CIPHERTEXT</b></div>}
                </div>
        ...
        <section className="section experience">
          ...
          <div className="timeline">
            <article><time>2024 — PRESENT</time><div><span>SOFTWARE ENGINEER</span><h3>Industrial & Real-Time Systems</h3><p>Developing communication software and desktop tools where predictable timing, robust I/O, and maintainable C++ architecture are product requirements.</p><ul><li>4 ms industrial communication cycle</li><li>Concurrent device orchestration</li><li>Profiling-led latency reduction</li></ul></div></article>
            <article><time>2022 — 2024</time><div><span>COMPUTER SCIENCE</span><h3>Systems Foundations</h3><p>Built a practical base across data structures, operating systems, networking, databases, and computer graphics through implementation-heavy projects.</p></div></article>
          </div>
        </section>

        <section id="contact" className="contact-section">
          <div className="contact-copy"><span>06 / OPEN CHANNEL</span><h2>Work with<br />Vaibhav Gaikwad.</h2><p>I’m interested in roles and collaborations involving C++, systems, graphics, networking, and performance-sensitive software.</p></div>
          <div className="contact-links">
            ...
            <a href="https://www.linkedin.com/search/results/people/?keywords=Vaibhav%20T%20Gaikwad" target="_blank" rel="noreferrer"><span><Icon name="linkedin" /> LINKEDIN</span><b>Find Vaibhav T Gaikwad</b><Icon name="arrow" /></a>
          </div>
        </section>
      </main>
      ...
      {selected && <div className="modal-wrap" role="dialog" aria-modal="true" aria-label={`${selected.title} case study`} onMouseDown={e => e.target === e.currentTarget && setSelected(null)}>
        <div className="modal">
          ...
          <div className="modal-stack">{selected.stack.map(x=><code key={x}>{x}</code>)}</div>
        </div>
      </div>}
